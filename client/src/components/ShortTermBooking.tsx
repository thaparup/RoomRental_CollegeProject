...
export default function ShortTermBooking() {
  const getAllRoomShortTermBooking = async () => {
    try {
  ...
  const { data: roomData } = useQuery(["getAllRoomShortTermBooking"], getAllRoomShortTermBooking);

  async function getImageRoomShortTermBooking() {
    try {
  ...
  const { data: imageData } = useQuery(["getImageRoomShortTermBooking"], getImageRoomShortTermBooking);
        ...
        {roomData?.data.length === 0 ? (
          ...
          >
            {roomData?.data.map((item) => {
              if (item.roomtype !== "SHORT_TERM_BOOKING") {
